...
### 19:01 - Nezha 发布监控的重启语义

- 问题或设计问题：新增 Nezha release 监控后，需要明确服务重启是否会重复通知，以及首次运行如何从用户指定的 `v2.2.6` 基线开始判断。
- 相关上下文或约束：项目已经有 `task_states.metadata_json` 和 `dedupe_keys`，CodexRadar 也复用 metadata 保存小状态；用户要求不新增数据库表、不新增依赖，并且任务每 12 小时检查 GitHub 最新稳定 release。
- 考虑过的方案：新增专用表能让版本状态更显式，但会扩大迁移和维护面；完全依赖通知 dedupe key 能防止已发送消息重复，但 job 本身仍会在每次运行构造同一通知；只用内存状态无法覆盖重启。
- 最终决策和理由：复用现有任务 metadata 保存最新已见 tag，并让通知 dedupe key 使用 `nezha-release:<tag>`。首次运行没有 metadata 时以代码内固定 `v2.2.6` 为基线；稳定 release 高于基线才通知。这样重启后不会重复推送同一版本，也不需要 schema 变更。
- 剩余权衡、风险或后续工作：如果持久化 SQLite 数据丢失，任务会重新按 `v2.2.6` 判断，可能补发当前最新稳定版通知；这是可接受的恢复行为。当前只解析简单语义化 tag，若 Nezha 未来改用非标准稳定 tag，需要扩展比较逻辑。
